...
use App::Yath2::Schema::Util qw/qdb_driver dbd_driver format_driver/;
my $version = App::Yath2::Schema::Util->VERSION;
            ...
            'App::Yath2::Schema',
            {
                ...
                rel_name_map => sub {
                    ...
                    return 'children'
                        ...
                        && $info->{local_columns}
                        && $info->{name}                eq 'events'
                        && $info->{local_class}         eq 'App::Yath2::Schema::Result::Event'
                        && $info->{remote_class}        eq 'App::Yath2::Schema::Result::Event'
...
#    $extra .= "use DBIx::Class::Storage::DBI::MariaDB;\n" if $schema =~ m/(MySQL|Percona|MariaDB)/i;
    ...
    system("rm -rf lib/App/Yath2/Schema/${schema}");
    mkdir "lib/App/Yath2/Schema/${schema}";
    open(my $fh, '>', "lib/App/Yath2/Schema/${schema}.pm") or die "Could not open file: $!";
    print {$fh} <<"    EOT";
package App::Yath2::Schema::${schema};
use utf8;
use strict;
use warnings;
use Carp();
...
Carp::confess("Already loaded schema '\$App::Yath2::Schema::LOADED'") if \$App::Yath2::Schema::LOADED;

\$App::Yath2::Schema::LOADED = "${schema}";
...
App::Yath2::Schema::${schema} - Autogenerated schema file for ${schema}.
...
=cut
    ...
    opendir(my $dh, "tmp/$schema/App/Yath2/Schema/Result/") or die "Could not open dir: $!";
    for my $file (sort readdir($dh)) {
        next unless $file =~ m/(.+)\.pm$/;
        my $pkg = $1;

        my $dest = "lib/App/Yath2/Schema/${schema}/$file";
        print "Found ${file}\n";

        my $from = "tmp/$schema/App/Yath2/Schema/Result/$file";
        process_uuid($from);
...
App::Yath2::Schema::${schema}::${pkg} - Autogenerated result class for ${pkg} in ${schema}.

=head1 SEE ALSO

L<App::Yath2::Schema::Overlay::${pkg}> - Where methods that are not
...
=cut
        ...
        my $result = "lib/App/Yath2/Schema/Result/$file";
        open(my $oh, '>', $result) or die "Could not open result file: $!\n";
        print $oh <<"        EOT";
package App::Yath2::Schema::Result::$pkg;
use utf8;
use strict;
use warnings;
...
require "App/Yath2/Schema/\${App::Yath2::Schema::LOADED}/${pkg}.pm";
require "App/Yath2/Schema/Overlay/${pkg}.pm";
...
App::Yath2::Schema::Result::$pkg - Autogenerated result class for $pkg.
...
=cut
        ...
        my $override = "lib/App/Yath2/Schema/Overlay/$file";
        unless (-e $override) {
            ...
            $pkg =~ s/\.pm$//;
            print $oh <<"            EOT";
package App::Yath2::Schema::Overlay::$pkg;
our \$VERSION = '$version';

package
    App::Yath2::Schema::Result::$pkg;
use utf8;
use strict;
use warnings;
...
App::Yath2::Schema::Overlay::$pkg - Overlay for $pkg result class.
...
sub process_uuid {
    ...
    while (my $line = <$fh>) {
        ...
        if ($line =~ m/use base 'DBIx::Class::Core';/) {
            push @lines => "use parent 'App::Yath2::Schema::ResultBase';\n";
            next;
        }
    ...
    if (@uuids) {
        my $specs = join "\n" => map { "__PACKAGE__->inflate_column('$_' => { inflate => \\&App::Yath2::Schema::Util::format_uuid_for_app, deflate => \\&App::Yath2::Schema::Util::format_uuid_for_db });" } @uuids;
        print $fh "$specs\n";
    }
