...
for my $set (qw/from to/) {
    option_group {group => $set, prefix => $set, category => ucfirst($set) . " Database Options"} => sub {
        option config => (
            type          => 'Scalar',
            description   => "Module that implements 'MODULE->yath_db_config(%params)' which should return a App::Yath2::Schema::Config instance.",
...
sub run {
    ...
    my $sync = App::Yath2::Schema::Sync->new();
