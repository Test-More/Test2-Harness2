...
use App::Yath2::Schema::Util qw/schema_config_from_settings format_uuid_for_db/;
use Test2::Util::UUID qw/gen_uuid/;
use App::Yath2::Schema::ImportModes qw/is_mode/;
...
# Our command dispatcher does not call accepts_dot_args/set_dot_args (those are
# part of the pre_ai base). Instead it hands the command the full trailing argv
# including any '::' stop token. Pull launcher args out of args() here so they
# land in webserver.launcher_args, matching set_dot_args() behavior above.
sub parse_args {
    my $self = shift;
    ...
    push @{$settings->webserver->launcher_args} => @dot_args if @dot_args;
...
sub run {
    ...
    my $server = $self->{+SERVER} = App::Yath2::Server->new(schema_config => $config, $settings->webserver->all, qdb_params => $qdb_params);
    $server->start_server;
