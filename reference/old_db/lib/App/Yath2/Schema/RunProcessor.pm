package App::Yath2::Schema::RunProcessor;
use strict;
use warnings;
use utf8;
...
use App::Yath2::Schema::Util qw/format_duration is_invalid_subtest_name schema_config_from_settings format_uuid_for_db/;
use App::Yath2::Renderer::Default::Composer();
use Test2::Util::UUID qw/gen_uuid/;

use Test2::Harness2::Util::JSON qw/encode_json decode_json/;

# Test2::Harness2::Util::JSON does not export a decode_json_no_null variant the
# way the legacy UI dist did, so carry a local copy. It escapes embedded NULs
# before decoding so databases that cannot store a NUL character (postgres) do
# not choke. For drivers that can store NUL (sqlite/mysql) we never call this.
...
sub do_decode_json {
    state $DECODER //= App::Yath2::Schema::can_store_null_character() ? \&decode_json : \&decode_json_no_null;
    return $DECODER->(@_);
}

use App::Yath2::Schema::ImportModes qw{
    %MODES
    record_all_events
    event_in_mode
    mode_check
...
sub _process_first_line {
    ...
    {
        no warnings 'once';
        $dbh->{mysql_auto_reconnect} = 1 if $App::Yath2::Schema::LOADED =~ m/(mysql|percona|maraidb)/i;
    }
    ...
    if (my $runf = $f->{harness_run}) {
        ...
        # Our collector emits the project name under the 'harness' settings
        # group (settings.harness.project); the legacy UI dist read it from a
        # 'yath' group that our logs do not have. Try all known locations and
        # fall back to a default rather than dying.
        my $log_settings = $runf->{settings} // {};
        my $proj
            = $log_settings->{harness}->{project}
            // $log_settings->{yathui}->{project}
            // $log_settings->{yath}->{project}
            // $params{project}
            // ($settings->check_group('yath') ? $settings->yath->project : undef)
            // 'default';
...
sub _pull_facet_job_try_updates {
    ...
    if (my $job_end = $f->{harness_job_end}) {
        ...
        # Our collector emits harness_job_end.times.total directly; the legacy
        # UI dist expected harness_job_end.times.totals.total. Accept both.
        my $duration = 0;
        if (my $times = $job_end->{times}) {
            $duration = $times->{total} // ($times->{totals} ? $times->{totals}->{total} : undef) // 0;
        }
...
sub process_event {
    ...
    # Our collector does not put a 'stamp' on the harness facet (it lives at the
    # record top-level). Capture it so _process_event can fall back to it.
    my $rec_stamp = $event->{stamp};

    my $ok = eval {
        my @todo = ([$f, event => $event]);
        while (my $set = shift @todo) {
            my ($sf, %sp) = @$set;
            push @todo => $self->_process_event($sf, %sp, rec_stamp => $rec_stamp, job => $job, try => $try, idx => $idx, sdx => $sdx++);
        }
...
sub _process_event {
    ...
    unless ($formatted_stamp) {
        ...
        # Our logs carry the timestamp at the record top-level rather than on
        # the harness facet; use it before falling back to the last-seen stamp.
        $formatted_stamp //= $self->format_stamp($params{rec_stamp}) if $params{rec_stamp};

        $formatted_stamp //= $self->format_stamp($self->{+LAST_STAMP}) if $self->{+LAST_STAMP};
    }

    my $rendered = App::Yath2::Renderer::Default::Composer->render_super_verbose($f);
    ...
    return map {[$_, job => $job, try => $try, idx => $idx, rec_stamp => $params{rec_stamp}, parent => $e_uuid, orphan => $orphan]} @$children;
}
...
App::Yath2::Schema::RunProcessor - Processes runs for database import
