...
use App::Yath2::Schema::Util qw/qdb_driver dbd_driver/;
...
sub start_ephemeral_db {
    ...
    require(mod2file("App::Yath2::Schema::$schema_type"));
...
sub _do_server_post_exec {
    ...
    my $app = App::Yath2::Server::Plack->new(
        schema_config => bless($data->{+SCHEMA_CONFIG}, 'App::Yath2::Schema::Config'),
    );
...
sub start_importers {
    ...
    for (1 .. $self->{+IMPORTERS} // 2) {
        push @pids => App::Yath2::Schema::Importer->new(config => $self->{+SCHEMA_CONFIG})->spawn();
    }
