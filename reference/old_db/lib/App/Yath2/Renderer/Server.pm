...
use parent 'App::Yath2::Renderer::DB';
...
include_options(
    ...
    'App::Yath2::Options::Server' => [qw/ephemeral/],
);
...
# pre_ai had a dedicated exit_hook() called by its renderer base. OUR pipeline
# only calls finish(), so the server teardown + "press ENTER" interaction that
# used to live in exit_hook() now runs at the end of finish(), after the DB
# renderer's importer teardown.
sub finish {
    my $self = shift;
...
App::Yath2::Renderer::Server - Renderer that runs an ephemeral server while
