...
- `Test2::Harness2::Collector` — the entry point and the IO loop.
  ...
  auditor → recorder pipeline, reaps the child, finalises.
...
- Bulk-insert optimisation — `insert(table, \%row1, \%row2, …)` must
...
- Wire the DB recorder from Stage 3: `Test2::Harness2::Collector::Recorder::DB`
  ...
  on finalise. Replace the `croak "not yet implemented"` placeholders.

Optional aids:

- `SQL::Abstract` for query construction where it pays for itself.
- Collectors and auditors may issue SQL on their own `DBI` handle
  directly; they don't have to go through the row-object layer for
  hot paths.
...
- Which of the ported resources need their own service process under
  ...
  any genuinely need a backing service service?
...
- Default behaviour when reload fails: stay broken until manual
...
- Edge-case auditor / parser behaviour we glossed over.
...
- Confirm the function-length cap (75 executable lines per sub) is
  honoured everywhere. Port `reference/old3`'s
  `author/find-long-subs` script as a tripwire.
- Confirm the module-length cap (1000 lines of code per `.pm`,
  excluding POD) is honoured. Any module over the cap gets flagged
