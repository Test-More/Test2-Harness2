...
- **Part 2 — `App::Yath2`.** User-facing `yath` command, options layer,
  output pipeline, persistent daemons, log archive / database backends.
  See `PART_2_PLAN.md` (scaffold; will be filled in once Part 1 is
  complete).
...
`ARCHITECTURE.md`, `STYLE_GUIDE.md`, or `new_plan`, follow the
current docs and **stop and ask** if the conflict is non-trivial.
...
5. **`new_plan`** — The original specification this rewrite was
   chartered from. Treat as foundational context. Where it conflicts
   with `ARCHITECTURE.md`, follow `ARCHITECTURE.md` (which exists to
   resolve such conflicts) — but flag the inconsistency so the docs
   can be reconciled.
6. **This file** — Per-repository agent / contributor preferences.
...
etc.). If the rule or behaviour matters to the user, restate it
