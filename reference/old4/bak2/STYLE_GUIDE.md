...
on signal interruption (`EINTR`), which is the behaviour we want:
...
- Be brief. POD should describe behaviour the reader can't infer
...
- POD **must not** reference any `.md` document — not
  ...
  Restate the relevant rule or behaviour in plain prose if it
  matters.
