...
# Start daemon with JSONFile protocol so the FD-pass path is not available.
yath(
    command => 'start',
    args    => [
        "--workdir=$dir",
        '--ipc-protocol=IPC::Manager::Client::JSONFile',
        '-R', 'Test2::Harness2::Resource::StagePreloader',
        "--dev-lib=$lib",
    ],
    exit => 0,
);
...
yath(
    ...
    exit    => sub { $_[0] != 0 },
    test    => sub {
        like($_[0]->{output}, qr/ConnectionUnix/, 'error names required transport');
    },
);
