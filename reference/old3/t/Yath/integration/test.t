...
use Test2::V0;
plan skip_all => "TODO: --ext/--exclude-file/--exclude-list/--durations options not implemented";
__END__
    ...
    if ( eval{ symlink('nothing-there', $symlink) } ) {
        ...
        yath(
            ...
            test => sub {
                ...
                like($out->{output}, qr{PASSED.*\Qt/Yath/integration/test-broken-symlinks/pass.tx\E}, q[t/Yath/integration/test-broken-symlinks/pass.tx PASSED]);
            },
        );
    }
}
