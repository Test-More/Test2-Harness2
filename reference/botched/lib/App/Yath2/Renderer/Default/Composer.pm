package App::Yath2::Renderer::Default::Composer;
use strict;
use warnings;
...
App::Yath2::Renderer::Default::Composer - Compose output components from event facets

=head1 DESCRIPTION

This is used by L<App::Yath2::Renderer::Default> to turn events into output
...
All methods are class methods, but they also work on a blessed instance.

=over 4

=item $inst = $class->new()

Create a blessed instance for convenience.

=item $arrayref = $class->render_one_line($event)

=item $arrayref = $class->render_verbose($event, %params)

=item $arrayref = $class->render_super_verbose($event)

=item $arrayref = $class->render_brief($event)
...
=item @lines = $class->render_control(\%facet_data, %params)
