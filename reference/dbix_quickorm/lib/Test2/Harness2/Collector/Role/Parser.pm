...
Test2::Harness2::Collector::Role::Parser - Role implemented by collector
parsers.

=head1 DESCRIPTION

Parsers sit at the front of the collector pipeline. The collector reads
bytes off the collected process's STDOUT and STDERR pipes and hands each
line (or each pre-decoded event burst) to the parser; the parser turns the
input into zero or one L<Test2::Harness2::Event> objects which are then
passed downstream to the optional processor and on to the events file.
...
emit). Named parameters always include C<stream> (C<'stdout'> or
C<'stderr'>) and at least one of:
...
The original zstd-compressed bytes of a message burst, for sinks that want
to fast-path the on-disk write without re-encoding.
