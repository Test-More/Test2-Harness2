...
Test2::Harness2::Util::Zstd::Writer - Append-safe, frame-per-record zstd
writer.

=head1 DESCRIPTION

Wraps an C<O_APPEND> filehandle. Each call to L</print> compresses the
joined payload as one self-contained zstd frame and atomically appends that
frame to the file via C<syswrite>. Concurrent writers are safe as long as a
frame fits in C<PIPE_BUF> (4 KiB on Linux); the per-line jsonl events the
harness emits stay well below that.

Construct via L<Test2::Harness2::Util::Zstd/open_zstd_writer>; this class is
...
Compresses the concatenation of C<@list> as one zstd frame and appends it.
...
Append a fully-formed zstd frame to the file without re-compressing. The
caller is responsible for ensuring the frame was produced with a matching
level so a reader can decode it. Used by callers that already hold a
compressed frame (e.g. the collector caches the on-wire frame from
L<Atomic::Pipe>).
...
sub print_raw_frame ($self, $frame) {
...
=item $w->_emit_frame($frame)

C<syswrite> one fully-formed zstd frame, croaking on error or short write.

=back

=cut

sub _emit_frame ($self, $frame) {
    my $fh   = $self->{fh};
    my $len  = length $frame;
