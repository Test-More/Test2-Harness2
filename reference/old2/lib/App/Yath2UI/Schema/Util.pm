package App::Yath2UI::Schema::Util;
use strict;
use warnings;
...
sub schema_config_from_settings {
    ...
    my $config_class = delete $params{config_class} // 'App::Yath2UI::Schema::Config';
    require(mod2file($config_class));

    my $group = $params{settings_group} // 'db';
    my $db = $settings->group($group);
    unless($db) {
        return App::Yath2UI::Schema::Config->new(%params) if $params{ephemeral};
    ...
    if ($dsn) {
        return App::Yath2UI::Schema::Config->new(
            %params,
            dbi_dsn  => $dsn,
            dbi_user => $db->user // '',
            dbi_pass => $db->pass // '',
    ...
    return App::Yath2UI::Schema::Config->new(%params);
}

{
    no strict 'refs';
    no warnings 'once';
    *{$_} = *{"App::Yath2UI::Schema::DBIC::$_"} for qw/is_mysql is_postgresql is_sqlite is_percona is_mariadb/;
}

sub format_uuid_for_db  { App::Yath2UI::Schema::DBIC->format_uuid_for_db(@_) }
sub format_uuid_for_app { App::Yath2UI::Schema::DBIC->format_uuid_for_app(@_) }
