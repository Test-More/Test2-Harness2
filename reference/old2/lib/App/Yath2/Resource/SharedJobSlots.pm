...
use Test2::Harness2::Util qw/find_in_updir/;
...
sub shared_post_process {
    ...
    push @{$resource->classes->{'App::Yath2::Resource::SharedJobSlots'} //= []} => (shared_jobs_config => $base_name);
}
...
sub init {
    ...
    my $sconf = App::Yath2::Resource::SharedJobSlots::Config->find(base_name => $self->{+SHARED_JOBS_CONFIG})
...
App::Yath2::Resource::SharedJobSlots - limit the job count (-j) per machine
...
instance of C<App::Yath2::Resource::SharedJobSlots::State>.
...
C<App::Yath2::Resource::SharedJobSlots::State> which implement the
