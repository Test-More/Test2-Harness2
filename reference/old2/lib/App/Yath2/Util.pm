...
sub find_yath {
    return $App::Yath2::Script::SCRIPT if defined $App::Yath2::Script::SCRIPT;

    if (-d 'scripts') {
        my $script = File::Spec->catfile('scripts', 'yath');
        return $App::Yath2::Script::SCRIPT = clean_path($script) if -e $script && -x $script;
    }
    ...
    for my $path (@Config{@keys}) {
        ...
        $App::Yath2::Script::SCRIPT = $script = clean_path($script);
        return $script;
    }
