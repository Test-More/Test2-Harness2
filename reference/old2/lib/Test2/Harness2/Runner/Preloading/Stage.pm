package Test2::Harness2::Runner::Preloading::Stage;
use strict;
use warnings;
...
use Test2::Harness2::IPC::Util qw/start_process ipc_connect ipc_warn ipc_loop pid_is_running inflate set_procname/;
    ...
    if ($do eq 'start') {
        ...
        my $ok = eval {
            ...
            if (my $test = $got->{run_test}) {
                $EXIT = Test2::Harness2::Collector::Preloaded->collect(%$test, orig_sig => \%ORIG_SIG, stage => $self, root_pid => $self->{+ROOT_PID});
            }
            elsif (my $spawn = $got->{run_spawn}) {
                $EXIT = Test2::Harness2::Collector::Preloaded->spawn(%$spawn, orig_sig => \%ORIG_SIG, stage => $self);
            }
...
sub do_preload {
    ...
    for my $mod (@$preloads) {
        ...
        $preload //= Test2::Harness2::Preload->new();
...
sub start_stage {
    ...
    local $Test2::Harness2::Runner::Preloading::Stage::ACTIVE;

    my $stage_ref = ref($stage);
    my ($name);
    if ($stage_ref) {
        $Test2::Harness2::Runner::Preloading::Stage::ACTIVE = $stage;
        $name = $stage->name;
    }
    else {
        $name = $stage;
    ...
    ipc_loop(
        ...
        handle_request => sub {
            ...
            if ($api_call eq 'launch_job') {
                ...
                my $ts  = inflate($args->{test_settings}, 'Test2::Harness2::TestSettings') or die "No test_settings provided";
                my $run = inflate($args->{run},           'Test2::Harness2::Run')          or die "No run provided";
                my $job = inflate($args->{job},           'Test2::Harness2::Run::Job')     or die "No job provided";
                ...
                return Test2::Harness2::Instance::Response->new(
...
Test2::Harness2::Runner::Preloading::Stage - FIXME
