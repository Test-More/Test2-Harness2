package Test2::Harness2::IPC::Protocol::AtomicPipe;
use strict;
use warnings;

our $VERSION = '2.000011';

use Test2::Harness2::IPC::Protocol::AtomicPipe::Connection;
...
use Test2::Harness2::IPC::Util qw/check_pipe ipc_warn pid_is_running/;
use Test2::Harness2::Util::JSON qw/decode_json/;

use Test2::Harness2::Instance::Message;
use Test2::Harness2::Instance::Request;
use Test2::Harness2::Instance::Response;

use parent 'Test2::Harness2::IPC::Protocol';
...
sub connect {
    ...
    my $con = Test2::Harness2::IPC::Protocol::AtomicPipe::Connection->new(fifo => $fifo, reader => $self, protocol => $self->protocol);
...
sub send_message {
    ...
    $msg = Test2::Harness2::Instance::Message->new(%$msg)
        unless blessed($msg) && $msg->isa('Test2::Harness2::Instance::Message');
...
sub __read_messages {
    ...
    while (1) {
        ...
        if ($msg->isa('Test2::Harness2::Instance::Response')) {
            my $con = $self->connection_from_meta($ipc_meta);
            $con->handle_response($msg);
        }
        elsif ($msg->isa('Test2::Harness2::Instance::Request')) {
...
sub connection_from_meta {
    ...
    return $self->{+CONNECTIONS}->{$fifo} //= Test2::Harness2::IPC::Protocol::AtomicPipe::Connection->new(fifo => $fifo, reader => $self);
}
...
Test2::Harness2::IPC::Protocol::AtomicPipe - FIXME
