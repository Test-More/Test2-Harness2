...
sub init {
    ...
    unless (blessed($ts)) {
        my $class = delete $ts->{class} // 'Test2::Harness2::TestSettings';
    ...
    if (my $jobs = $self->{+JOBS}) {
        my (@jobs, %jobs);
        for my $job (@$jobs) {
            my $class = $job->{job_class} // 'Test2::Harness2::Run::Job';
...
sub ipc {
    ...
    return $self->{+IPC} = Test2::Harness2::IPC::Protocol->new(protocol => $agg_ipc->{protocol});
}
...
sub send_event_cb {
    ...
    $self->{+SEND_EVENT_CB} //= Test2::Harness2::Collector::Child->send_event();
}
...
sub send_event {
    ...
    $event = Test2::Harness2::Event->new($event)
