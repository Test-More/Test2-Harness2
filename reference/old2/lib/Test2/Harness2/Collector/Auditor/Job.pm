package Test2::Harness2::Collector::Auditor::Job;
use strict;
use warnings;
...
use Test2::Harness2::Log::TimeTracker;

use Test2::Harness2::Event;

use parent 'Test2::Harness2::Collector::Auditor';
...
sub init {
    ...
    $self->{+NUMBERS} = {};
    $self->{+TIMES} = Test2::Harness2::Log::TimeTracker->new();
...
sub update_summary {
    ...
    if (my $file = $self->{+SUMMARY_FILE}) {
        ...
        Test2::Harness2::Util::File::JSON->new(name => $file)->write($new);
    }
}
...
sub _audit {
    ...
    if ($f->{from_tap} && $f->{harness}->{subtest_end} && !($self->{+SUBTESTS} && keys %{$self->{+SUBTESTS}})) {
        ...
        $event = Test2::Harness2::Event->new(stamp => $stamp, job_id => $self->{+JOB_ID}, job_try => $self->{+JOB_TRY}, facet_data => $f);
    }
...
Test2::Harness2::Collector::Auditor::Job - Class to monitor events for a single job
...
=head1 SYNOPSIS
    ...
    my $auditor = Test2::Harness2::Collector::Auditor::Job->new();
...
Retuns the L<Test2::Harness2::Log::TimeTracker> instance.
