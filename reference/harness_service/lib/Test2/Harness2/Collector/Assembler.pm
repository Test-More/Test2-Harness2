...
sub process_event ($self, $event) {
    ...
    # Synthetic process-exit: the child is gone, so any TAP subtest still
    # buffered will never receive its closing brace (nor a shallower event to
    # close it implicitly). Force every open subtest closed, in arrival order,
    # ahead of the exit event itself.
    return ($self->_close_deeper_subtests($event, $self->{+NESTED}), $event)
...
sub _tap_process_event ($self, $event, $f, $nested) {
    ...
    if ($is_ours) {
        # Authoritative: any subtest its arrival just closed finished *before*
        # this event, so emit those first, then the event itself (unless it is
        # a closing brace, which is structure, not content).
        my @out;
        push @out => @closed;
        push @out => $event
