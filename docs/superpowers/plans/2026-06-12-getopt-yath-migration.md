...
- [ ] **Step 10.0 (added from Task 9 review):** Decide and implement `--help`/`-h` semantics: live behavior was `yath --help test` → test-specific help and bare `yath -h` → help-command table; the Task 9 interim shortcut renders option docs only. Restore live parity now that commands load: the Debug help post should defer to the help command when one resolves (or the resolution path should treat `--help <cmd>` as `help <cmd>`). Also note `load_command(check_only)` DIES on broken-compile commands — convert all commands in one commit to avoid cascading `use parent` chains (run→abort→status).
