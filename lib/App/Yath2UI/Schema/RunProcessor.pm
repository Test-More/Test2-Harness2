package App::Yath2UI::Schema::RunProcessor;
use strict;
use warnings;
use utf8;
...
use App::Yath2UI::Schema::Util qw/format_duration is_invalid_subtest_name schema_config_from_settings format_uuid_for_db/;
use Test2::Util::UUID qw/gen_uuid/;

use Test2::Harness2::Util::JSON qw/encode_json decode_json_no_null decode_json/;

sub do_decode_json {
    state $DECODER //= App::Yath2UI::Schema::DBIC::can_store_null_character() ? \&decode_json : \&decode_json_no_null;
    return $DECODER->(@_);
}

use App::Yath2UI::Schema::ImportModes qw{
    %MODES
    record_all_events
    event_in_mode
    mode_check
...
sub _process_first_line {
    ...
    {
        no warnings 'once';
        $dbh->{mysql_auto_reconnect} = 1 if $App::Yath2UI::Schema::DBIC::LOADED =~ m/(mysql|percona|maraidb)/i;
    }
...
App::Yath2UI::Schema::RunProcessor - Processes runs for database import
