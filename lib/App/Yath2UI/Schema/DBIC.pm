package App::Yath2UI::Schema::DBIC;
use utf8;
use strict;
use warnings;
...
if ($App::Yath2UI::Schema::DBIC::LOADED) {
    require App::Yath2UI::Schema::DBIC::ResultSet;
    __PACKAGE__->load_namespaces(
        default_resultset_class => '+App::Yath2UI::Schema::DBIC::ResultSet',
    );
}

sub is_sqlite     { ($App::Yath2UI::Schema::DBIC::LOADED // '') =~ m/SQLite/     ? 1 : 0 }
sub is_postgresql { ($App::Yath2UI::Schema::DBIC::LOADED // '') =~ m/PostgreSQL/ ? 1 : 0 }
sub is_mariadb    { ($App::Yath2UI::Schema::DBIC::LOADED // '') =~ m/MariaDB/    ? 1 : 0 }
sub is_percona    { ($App::Yath2UI::Schema::DBIC::LOADED // '') =~ m/Percona/    ? 1 : 0 }

sub is_mysql {
    return 1 if is_mariadb();
    return 1 if is_percona();
    return 1 if ($App::Yath2UI::Schema::DBIC::LOADED // '') =~ m/MySQL/;
    return 0;
}
...
App::Yath2UI::Schema::DBIC - Unified DBIx::Class schema class with per-backend helpers.
