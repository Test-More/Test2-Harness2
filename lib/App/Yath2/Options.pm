...
sub set_command_class {
    ...
    croak "Invalid command class: $class"
        unless $class->isa('App::Yath2::Command');
...
sub process_option_post_actions {
    ...
    if ($cmd) {
        ...
        croak "The process_option_post_actions requires an App::Yath2::Command instance, got: " . ($cmd // "undef")
            unless blessed($cmd) && $cmd->isa('App::Yath2::Command');
...
sub _parse_option_caller {
    ...
    if ($caller->isa('App::Yath2::Command')) {
        $from_command = $caller->name() unless $caller eq 'App::Yath2::Command';
        $is_top       = 1;
    }
    elsif ($caller =~ m/App::Yath2::Command::([^:]+)::.*Options(?:::.*)?$/) {
        $from_command = $1;
        $is_top       = 1;
    }
    elsif ($caller eq 'App::Yath2') {
        $is_top = 1;
    }
    elsif ($caller =~ m/^(App::Yath2::Plugin::([^:]+))$/) {
        $from_plugin = $1;
        $from_prefix = $2;
...
App::Yath2::Options - Tools for defining and tracking yath CLI options.
...
the option. See L<App::Yath2::Option> for the documentation of attributes.
...
Arrayref containing all the L<App::Yath2::Option> instances in the options
instance.

=item $settings = $options->settings

Get the L<Test2::Harness2::Settings> instance.
