package App::Yath2::DB::Internal::MySQL;
use strict;
use warnings;
...
sub _apply_session_state {
    my $self = shift;
    my $dbh = $self->{App::Yath2::DB::Internal::DBH} or return;
...
sub _create_archive {
    ...
    $self->{App::Yath2::DB::Internal::ARCHIVE_ID} = $id;
    $self->{App::Yath2::DB::Internal::UUID}       = $uuid;
    return $id;
}
...
sub _ensure_run_row {
    ...
    my $project_id = $self->{App::Yath2::DB::Internal::PROJECT_ID}
...
sub _mysql_insert_body {
    ...
    $self->{App::Yath2::DB::Internal::PROJECT_ID} = $project_id;
    ...
    $self->{App::Yath2::DB::Internal::_INSERT_SOURCE()} = $source;
...
App::Yath2::DB::Internal::MySQL - MySQL/Percona-backed Log reader / writer.

=head1 SYNOPSIS

    my $log = App::Yath2::DB::Internal::MySQL->new(
        dsn => 'dbi:MariaDB:database=yath;host=db.local',
    );
    # OR
    my $log = App::Yath2::DB::Internal::MySQL->new(dbh => $dbh, uuid => $u);
...
L<App::Yath2::DB::Internal> and provides only the MySQL-specific bits:
