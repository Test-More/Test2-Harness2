package App::Yath2::DB::Internal::Sqlite;
use strict;
use warnings;
...
sub _connect_dbh {
    ...
    if (!defined $dsn) {
        ...
        $self->{App::Yath2::DB::Internal::DSN} = $dsn;
    }
...
App::Yath2::DB::Internal::Sqlite - SQLite-backed Log reader / writer.

=head1 SYNOPSIS

    my $log = App::Yath2::Log->new(file => '/tmp/run.yath');
    # OR
    my $log = App::Yath2::DB::Internal::Sqlite->new(file => '/tmp/run.yath');
    # OR
    my $log = App::Yath2::DB::Internal::Sqlite->new(dbh => $dbh, uuid => $u);
...
SQLite-backed Log backend. Subclasses L<App::Yath2::DB::Internal>,
