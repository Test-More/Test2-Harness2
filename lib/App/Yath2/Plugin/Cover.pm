...
option_group {prefix => 'cover', group => 'cover', category => "Cover Options"} => sub {
    ...
    option aggregator => (
        ...
        normalize => sub { fqmod($_[0], 'Test2::Harness2::Log::CoverageAggregator') },
    );
...
sub coverage_handler {
    ...
    if (my $fd = $set->{facet_data}) {
        if ($data = $fd->{job_coverage}) {
            require Test2::Harness2::Log::CoverageAggregator::ByTest unless $INC{'Test2/Harness2/Log/CoverageAggregator/ByTest.pm'};
            $agg = 'Test2::Harness2::Log::CoverageAggregator::ByTest';
        }
        elsif($data = $fd->{run_coverage}) {
            require Test2::Harness2::Log::CoverageAggregator::ByRun unless $INC{'Test2/Harness2/Log/CoverageAggregator/ByRun.pm'};
            $agg = 'Test2::Harness2::Log::CoverageAggregator::ByRun';
        }
        else {
            return;
