...
option_group {group => 'db', prefix => 'db', category => "Database Options"} => sub {
    ...
    # Wire the DB renderer into our display->renderers Map. In pre_ai the DB
    # renderer was injected via the (separate) renderer-options machinery; we
    # have no such machinery -- renderers live in settings->display->renderers
    # as a Map shaped { '@' => [\@ordered_classes], Class => [\@args] } (see
    # App::Yath2::Options::Display). When the user has asked to publish to the
    # database (--db-publish), append the DB renderer to that Map so events get
    # streamed into the database. Users can also opt in explicitly with
    # '--renderer +App::Yath2::Renderer::DB'.
    option publish => (
        type        => 'Bool',
        default     => 0,
        description => 'Publish test events to the database as they happen (adds the App::Yath2::Renderer::DB renderer)',
    );
};
...
option_post_process 100 => sub {
    ...
    return unless $db->publish;

    # display group may not be present for every command; only wire when it is.
    ...
    my $class = 'App::Yath2::Renderer::DB';
    return if exists $renderers->{$class};
...
App::Yath2::Plugin::DB - Yath plugin for database-backed coverage, durations,
rerun and live publishing.
