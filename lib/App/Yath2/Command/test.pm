...
use Getopt::Yath;
include_options(
    App::Yath2::Command::start->option_modules,
    'App::Yath2::Command::run',
);
...
sub run {
    ...
    if (my $pt = $settings->runner->preload_threshold) {
        ...
        if ($tc < $pt) {
            ...
            $settings->runner->class('Test2::Harness2::Runner');
            $settings->runner->preloads([]);
            $settings->runner->preload_early(undef);
        }
    }

    return $self->App::Yath2::Command::start::run();
}
...
sub renderers {
    my $self = shift;
    return $self->{+RENDERERS} //= App::Yath2::Options::Renderer->init_renderers($self->settings);
}
...
sub collector {
    ...
    my $run_id    = $settings->run->run_id;
    my $parser    = Test2::Harness2::Collector::IOParser->new(job_id => 0, job_try => 0, run_id => $run_id, type => 'runner');
...
sub handle_event {
    ...
    bless($event, 'Test2::Harness2::Event');
