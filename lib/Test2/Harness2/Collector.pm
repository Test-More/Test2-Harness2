...
use Test2::Harness2::Collector::Auditor::Job;
use Test2::Harness2::Collector::IOParser::Stream;

use Test2::Harness2::Util qw/mod2file parse_exit open_file chmod_tmp/;
use Test2::Harness2::Util::JSON qw/decode_json encode_json decode_json_file/;
use Test2::Harness2::IPC::Util qw/pid_is_running swap_io start_process ipc_connect ipc_loop inflate set_procname/;
...
sub collect_command {
    ...
    my $parser = Test2::Harness2::Collector::IOParser->new(
        job_id  => 0,
        job_try => 0,
        run_id  => 0,
...
sub collect_job {
    ...
    my $ts  = inflate($params{test_settings}, 'Test2::Harness2::TestSettings') or die "No test_settings provided";
    my $run = inflate($params{run},           'Test2::Harness2::Run')          or die "No run provided";
    my $job = inflate($params{job},           'Test2::Harness2::Run::Job')     or die "No job provided";
    ...
    my $auditor = Test2::Harness2::Collector::Auditor::Job->new(%create_params);
    my $parser  = Test2::Harness2::Collector::IOParser::Stream->new(%create_params, type => 'test');
...
sub setup_child_output {
    ...
    require Test2::Harness2::Collector::Child;
    {
        no warnings 'once';
        $Test2::Harness2::Collector::Child::STDOUT_APIPE = $handles->{out_w};
        $Test2::Harness2::Collector::Child::STDERR_APIPE = $handles->{err_w} if $self->{+MERGE_OUTPUTS};
    }
...
Test2::Harness2::Collector - FIXME
