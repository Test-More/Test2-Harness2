package Test2::Harness2::Collector::JobDir;
use strict;
use warnings;
...
use Test2::Harness2::Util qw/maybe_read_file open_file apply_encoding/;
...
use Test2::Harness2::Collector::TapParser qw{
    parse_stdout_tap
    parse_stderr_tap
};
...
sub poll {
    ...
    return map {
        ...
        Test2::Harness2::Event->new(stamp => $stamp, %{$_});
    } @out;
}
...
my %FILE_MAP = (
    ...
    'exit'   => [EXIT_FILE,   'Test2::Harness2::Util::File::Value'],

    'event_timeout'     => [ET_FILE,  'Test2::Harness2::Util::File::Value'],
    'post_exit_timeout' => [PET_FILE, 'Test2::Harness2::Util::File::Value'],
);
...
sub try_open {
    ...
    unless ($self->{+OPEN_ERROR_SEEN}->{$path}++) {
        push @$errors => Test2::Harness2::Event->new(
            stamp      => time,
            job_id     => 0,
            job_try    => undef,
            event_id   => gen_uuid(),
...
Test2::Harness2 checks for timeouts at a configurable interval, if a test does
...
Sometimes tests will fork and then return. On supported systems Test2::Harness2
will start all tests with their own process group and will wait for the entire
group to exit before considering the test done. In these cases Test2::Harness2
...
Test2::Harness2::Collector::JobDir - Job Directory Parser, read events from an active
