package Test2::Harness2::Runner::Resource::SharedJobSlots::Config;
use strict;
use warnings;
...
sub algorithm {
    ...
    if ($algorithm =~ m/^(.*)::([^:]+)$/) {
        ...
        require Test2::Harness2::Runner::Resource::SharedJobSlots::State;
        ...
        die "'$short' is not a valid algorithm (in file '$self->{+CONFIG_FILE}' under host '$self->{+HOST}' key 'algorithm'). Must be 'fair', 'first', or a Fully::Qualified::Module::function_name."
            unless Test2::Harness2::Runner::Resource::SharedJobSlots::State->can($algorithm);
    }
...
Test2::Harness2::Runner::Resource::SharedJobSlots::Config - Config for shared job slots
