package Test2::Harness2::Runner::Preloader;
use strict;
use warnings;
...
sub init {
    ...
    if ($need_depmap) {
        require Test2::Harness2::Runner::DepTracer;
        $self->{+DTRACE} //= Test2::Harness2::Runner::DepTracer->new();
    }
    ...
    $self->{+RELOADER} = Test2::Harness2::Runner::Reloader->new(
        stat_min_gap     => 2,
    ...
    # which `yath watch` (a global runner.socket subscriber) renders.
    {
        ...
        setjump 'Stage-Collector' => sub {
            ...
            my $info = Test2::Collector::collect(
                ...
                # exit against the preload-root's own teardown; deferred.)
...
sub _preload_module {
    ...
    die "You cannot load a Test2::Harness2::Runner::Preload module from within another" if $self->{+DONE};

    $self->{+STAGED} //= do {
        require Test2::Harness2::Runner::Preload;
        Test2::Harness2::Runner::Preload->new();
    };
...
Test2::Harness2::Runner::Preloader - Preload logic.
