package Test2::Harness2::Auditor::Watcher;
use strict;
use warnings;
...
use Test2::Harness2::Auditor::TimeTracker;
...
sub init {
    ...
    $self->{+NUMBERS} = {};
    $self->{+TIMES} = Test2::Harness2::Auditor::TimeTracker->new();
...
sub process {
    ...
    if ($f->{from_tap} && $f->{harness}->{subtest_end} && !($self->{+SUBTESTS} && keys %{$self->{+SUBTESTS}})) {
        ...
        $event = Test2::Harness2::Event->new(stamp => time, job_try => $self->{+TRY}, facet_data => $f);
    }
...
sub subtest_process {
    ...
    $event ||= Test2::Harness2::Event->new(facet_data => $f, job_try => $self->{+TRY});
...
Test2::Harness2::Auditor::Watcher - Class to monitor events for a single job and
...
=head1 SYNOPSIS
    ...
    use Test2::Harness2::Auditor::Watcher;

    my $watcher = Test2::Harness2::Auditor::Watcher->new();
...
Retuns the L<Test2::Harness2::Auditor::TimeTracker> instance.
