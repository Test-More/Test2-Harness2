package Test2::Harness2::IPC::Util;
use strict;
use warnings;
...
use Test2::Harness2::Util::JSON qw/encode_pretty_json encode_json encode_json_file/;
...
sub start_collected_process {
    ...
    my $pid = start_process(
        [
            ...
            '-mTest2::Harness2::Collector',                                  # Load Collector
            '-e' => 'exit(Test2::Harness2::Collector->collect($ARGV[0]))',   # Run it.
...
sub ipc_connect {
    ...
    require Test2::Harness2::IPC::Protocol;
    my $ipc = Test2::Harness2::IPC::Protocol->new(protocol => $ipc_data->{protocol});
