...
sub process {
    ...
    while (my $line = <STDIN>) {
        ...
        my $e = Test2::Harness2::Event->new($data);
...
sub process_event {
    ...
    if (my $job = $f->{harness_job}) {
        $tries->[$job_try] = Test2::Harness2::Auditor::Watcher->new(job => $job, try => $job_try);
    }
...
Test2::Harness2::Auditor - Auditor that validates test results by processing an
event stream.

=head1 DESCRIPTION

The auditor is responsible for taking a stream of events and determining what
is passing or failing. An L<Test2::Harness2::Auditor::Watcher> instance is
