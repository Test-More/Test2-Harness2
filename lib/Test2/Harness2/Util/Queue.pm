package Test2::Harness2::Util::Queue;
use strict;
use warnings;
...
sub seek {
    ...
    $self->{+QH} ||= Test2::Harness2::Util::File::JSONL->new(name => $self->{+FILE});
    $self->{+QH}->seek($pos);
...
sub _enqueue {
    ...
    my $fh = Test2::Harness2::Util::File::JSONL->new(name => $self->{+FILE}, use_write_lock => 1);
    $fh->write($task);
}
...
Test2::Harness2::Util::Queue - Representation of a queue.
...
=head1 SYNOPSIS
    ...
    my $queue = Test2::Harness2::Util::Queue->new(file => '/path/to/queue.jsonl');
