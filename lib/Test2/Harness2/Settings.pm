package Test2::Harness2::Settings;
use strict;
use warnings;
...
use Test2::Harness2::Settings::Prefix;
...
sub new {
    ...
    if (@_ == 1) {
        ...
        my $settings_file = Test2::Harness2::Util::File::JSON->new(name => $_[0]);
    ...
    for my $key (keys %$hash) {
        ...
        if (Scalar::Util::blessed($val)) {
            Carp::croak("All prefixes must contain instances of Test2::Harness2::Settings::Prefix")
                unless $val->isa('Test2::Harness2::Settings::Prefix');
        ...
        $hash->{$key} = Test2::Harness2::Settings::Prefix->new(%$val);
    }
...
sub define_prefix {
    ...
    return ${$self}->{$prefix} //= Test2::Harness2::Settings::Prefix->new;
}
...
Test2::Harness2::Settings - Configuration settings for Test2::Harness2.
...
=head1 SYNOPSIS
    ...
    # component of Test2::Harness2 will expose them to you.
...
See L<Test2::Harness2::Settings::Prefix> for more details on how to use the prefixes.
