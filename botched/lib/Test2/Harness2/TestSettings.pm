package Test2::Harness2::TestSettings;
use strict;
use warnings;
...
use Test2::Harness2::Util::HashBase('<cleared', map { "+$_" } @ARRAY, @HASH, @SCALAR, @BOOL);
...
Test2::Harness2::TestSettings - Configuration for how to run a test

=head1 DESCRIPTION

Encapsulates all configuration settings that control how a test is executed,
including library paths, timeouts, forking, preloading, and more. Supports
merging multiple settings instances with last-defined-wins for scalars,
deduplicated concatenation for arrays, and overlay for hashes.
...
=head1 SYNOPSIS
    ...
    my $merged = Test2::Harness2::TestSettings->merge($base, $override);
...
Class or instance method. Merges multiple TestSettings objects into a new one.
Scalars and booleans use last-defined-wins. Arrays are concatenated with
deduplication. Hashes are overlaid. The C<cleared> field can remove inherited
settings.

=item default_cover_args

Returns the default cover argument string used when C<cover> is set to C<1>.
