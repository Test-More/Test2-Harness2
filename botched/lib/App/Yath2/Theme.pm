...
sub assign_job_color {
    ...
    for my $color (shuffle($self->DEFAULT_JOB_COLORS())) {
        my $c = $self->{+_JOB_COLOR_INSTANCES}->{$color} //= 0;
    ...
    # We now either have an unused color, or one of the least used colors.
    $self->{+_JOB_COLOR_INSTANCES}->{$it}++;    # Note another use of the color

    $self->{+COLOR_MAP_TEXT}->{job}->{$id} = $it;
    my $ok  = eval { $self->{+COLOR_MAP_TERM}->{job}->{$id} = color($it); 1 };
    my $err = $@;
    die "Failed to assign job color '$it': $err" unless $ok;
...
App::Yath2::Theme - Color and style theme system for terminal output

=head1 DESCRIPTION

The Theme class manages ANSI color codes for terminal output. It provides
default color maps for base colors, state colors (pass/fail), status colors,
tag colors, facet colors, and job colors. It assigns rotating colors to
concurrent jobs.
...
=head1 SYNOPSIS
    ...
    my $reset = $theme->get_term_color('base', 'reset');
    my $borders = $theme->get_borders('default');

    $theme->assign_job_color('job-001');
    my $color = $theme->get_term_color('job', 'job-001');
