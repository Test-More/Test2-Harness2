...
use Test2::Harness2::Util::JSON qw/encode_pretty_json json_true json_false/;
...
sub exit_hook {
    ...
    my ($final_data, $summary);

    if (ref($auditor) eq 'HASH') {
        $final_data = $auditor->{final_data};
...
sub render_summary {
    ...
    return if $self->quiet && $self->quiet > 1;
...
App::Yath2::Renderer::Summary - Exit-time summary tables and optional JSON summary file

=head1 DESCRIPTION

This renderer generates summary tables at exit time showing pass/fail counts,
file and assertion totals, timing, and CPU usage. It can optionally write a
JSON summary file.
...
=head1 SYNOPSIS
    ...
    my $renderer = App::Yath2::Renderer::Summary->new(file => 'summary.json');
