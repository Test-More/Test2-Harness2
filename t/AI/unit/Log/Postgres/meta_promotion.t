...
for_each_db_version([qw/postgresql/], sub {
    ...
    # No meta.json artifact row written.
    ...
    # Postgres' DBD::Pg may or may not auto-decode JSONB depending on
    # settings; handle either decoded hash or raw JSON text.
    my $extras = ref($row->{meta_extras}) eq 'HASH'
    ? $row->{meta_extras}
    : decode_json($row->{meta_extras});
