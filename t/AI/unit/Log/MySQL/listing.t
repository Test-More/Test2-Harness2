...
my $writer = App::Yath2::Log::MySQL->new(dsn => $dsn);
...
# Multi-archive: insert another and verify the ambiguity error.
{
    ...
    my $w2 = App::Yath2::Log::MySQL->new(dsn => $dsn);
    $w2->insert(App::Yath2::Log->new(dir => $src2));

    like(
        dies { App::Yath2::Log::MySQL->new(dsn => $dsn)->runs },
        qr/ambiguous; specify uuid =>/,
        'multi-archive without uuid throws',
    );

    my $u = $w2->uuid;
    my $picked = App::Yath2::Log::MySQL->new(dsn => $dsn, uuid => $u);
    isa_ok($picked, ['App::Yath2::Log::MySQL']);
}
...
# 0-archive case: a fresh empty DB is allowed; reads throw.
{
    ...
    my $w = App::Yath2::Log::MySQL->new(dsn => $empty);
    isa_ok($w, ['App::Yath2::Log::MySQL']);
