...
# Unit-style test for App::Yath2::Log::DB::Postgres::_preprocess_schema_sql.
...
sub with_compression {
    my ($algo, $code) = @_;
    my $db = bless {}, 'App::Yath2::Log::DB::Postgres';
    no warnings 'redefine';
    local *App::Yath2::Log::DB::Postgres::_server_compression = sub { $algo };
    $code->($db);
}
...
subtest zstd_supported_leaves_clauses_unchanged => sub {
    with_compression('zstd', sub {
        my $db = shift;
        my $out = $db->_preprocess_schema_sql($sample);
...
subtest case_insensitive_match => sub {
    with_compression('lz4', sub {
        ...
        my $out = $db->_preprocess_schema_sql($mixed);
...
subtest leaves_other_compression_alone => sub {
    with_compression('lz4', sub {
        ...
        my $out = $db->_preprocess_schema_sql($sql);
