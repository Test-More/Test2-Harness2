...
# the same SQLite forces the caller to pass uuid => ... on reopen.
{
    ...
    my $writer = App::Yath2::Log::DB::Sqlite->new(file => $arc_path);
    ...
    isa_ok($picked, ['App::Yath2::Log::DB::Sqlite']);
}
...
# before the first insert), but reads throw "no archives in this DB".
{
    ...
    my $empty = "$empty_dir/empty.yath";
    my $writer = App::Yath2::Log::DB::Sqlite->new(file => $empty);
    isa_ok($writer, ['App::Yath2::Log::DB::Sqlite']);
