...
for_each_log_db_backend(sub {
    ...
    # the same SQLite forces the caller to pass uuid => ... on reopen.
    {
        ...
        my $writer = App::Yath2::DB->open(file => $arc_path, backend => $backend);
    ...
    # before the first insert), but reads throw "no archives in this DB".
    {
        ...
        my $writer = App::Yath2::DB->open(file => $empty, backend => $backend);
        ok(defined $writer, 'open on fresh empty file succeeds');
