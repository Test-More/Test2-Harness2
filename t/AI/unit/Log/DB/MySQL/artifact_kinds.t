...
# Schema-side defense: the ENUM rejects dropped values. Force strict
# mode so out-of-range ENUM values are hard errors instead of warnings.
...
for my $bad (qw(spec report state)) {
    ...
    my $ok = eval {
        ...
        # 16 raw bytes for a fresh UUID; doesn't have to be canonical.
        $sth->bind_param(2, "\x01" x 16, DBI::SQL_BINARY());
