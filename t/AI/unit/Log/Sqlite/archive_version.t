...
my $writer = App::Yath2::Log::Sqlite->new(dsn => "dbi:SQLite:$db_path");
...
# A fresh reader on the same DB resolves the archive at construction.
{
    my $reader;
    ok(
        lives { $reader = App::Yath2::Log::Sqlite->new(dsn => "dbi:SQLite:$db_path") },
...
like(
    dies { App::Yath2::Log::Sqlite->new(dsn => "dbi:SQLite:$db_path") },
    qr/refusing to read/,
...
ok(
    lives { App::Yath2::Log::Sqlite->new(dsn => "dbi:SQLite:$db_path") },
