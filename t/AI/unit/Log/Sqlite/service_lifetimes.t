...
# Helper: write multi-row JSONL.zst.
sub write_jsonl_zst {
    my ($path, @rows) = @_;
...
# Build a synthetic source with a global 'harness' service that
# restarted twice (3 report rows, 1 spec row) plus minimal run / job
# scaffolding so the run/job invariants are satisfied.
sub build_log_with_restarts {
    ...
    # Minimal run / job scaffolding to satisfy invariants.
...
unlink $db_path;
my $db = App::Yath2::Log::Sqlite->new(dsn => "dbi:SQLite:$db_path");
$db->bootstrap_schema;
my $dbh = $db->dbh;
...
# services row exists, role populated from spec.
...
# 3 service_lifetimes rows in order.
...
# Status: all three completed (each report has ended_at).
...
# Type and id promoted from spec on row 1.
...
# spec_extras on row 1 should contain non-promoted keys (pid, workdir, role).
...
# Rows 2 and 3 had no spec -> spec_extras NULL.
is($rows->[1]{spec_extras}, undef, 'row 2 spec_extras NULL');
is($rows->[2]{spec_extras}, undef, 'row 3 spec_extras NULL');

# state_extras on each report should contain non-promoted keys (id, type).
...
# times: report wins on collision -- decoded should match the report values.
...
# services no longer carries spec/state/status columns.
