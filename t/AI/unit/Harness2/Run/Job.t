...
subtest 'job_id is required' => sub {
    my $tf  = Test2::Harness2::TestFile->new(file => 't/foo.t');
    my $ok  = eval { Test2::Harness2::Run::Job->new(test_file => $tf, run_id => 0); 1 };
    my $err = $@;
...
subtest 'test_file is required' => sub {
    my $ok  = eval { Test2::Harness2::Run::Job->new(run_id => 0, job_id => 0); 1 };
    my $err = $@;
...
subtest 'run_id is required' => sub {
    my $tf = Test2::Harness2::TestFile->new(file => 't/foo.t');
    my $ok  = eval { Test2::Harness2::Run::Job->new(test_file => $tf, job_id => 0); 1 };
    my $err = $@;
...
subtest 'test_file_abs / test_file_rel shortcuts' => sub {
    my $tf  = Test2::Harness2::TestFile->new(file => 't/foo.t');
    my $job = Test2::Harness2::Run::Job->new(test_file => $tf, run_id => 0, job_id => 0);
