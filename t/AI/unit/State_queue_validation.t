...
# #118 ingress backstop. _queue_task is the one funnel every task passes through
# (queue_task / _retry_task / requeue_task all reach it), including the un-eval'd
...
subtest invalid_category_and_duration_normalized => sub {
    ...
    my $warnings = warnings { $state->_queue_task($task) };
...
subtest well_formed_task_is_silent => sub {
    ...
    is(warnings { $state->_queue_task($task) }, [], "a valid task warns nothing");
...
subtest one_bad_task_does_not_reap_sibling_runs => sub {
    ...
    $state->_queue_task($good);

    my $bad = mk_task(job_id => 'BAD', run_id => 'R1', category => 'network', duration => 'nope');
    my $lived = 0;
    my $warnings = warnings { $lived = lives { $state->_queue_task($bad) } };
    ok($lived, "a bad task never dies out of _queue_task") or diag($@);
