...
# The synthetic process-exit event the collector appends after the child exits.
# Not from_tap / from_stream -- it carries no nesting.
sub proc_exit () {
    return {harness_process_exit => {all => 0, err => 0, sig => 0, dmp => 0}};
}
...
subtest implicit_close_orders_subtest_before_closer => sub {
    my $as = Test2::Harness2::Collector::Assembler->new;

    # A streamed TAP subtest closed implicitly by a shallower plan line. The
    # subtest finished (its last child arrived) BEFORE the plan, so the
    # assembled subtest event must be recorded ahead of the plan that closed it.
    my @out = run(
        $as,
        tap_open('st', 0),
        tap_assert(1, number => 1, name => 'child a', nested => 1),
    ...
    my ($asm_idx) = grep { $out[$_]->facet_data->{parent} && $out[$_]->facet_data->{parent}{children} } 0 .. $#out;
    my ($plan_idx) = grep { $out[$_]->facet_data->{plan} } 0 .. $#out;

    ok(defined $asm_idx, "assembled subtest emitted");
    ok(defined $plan_idx, "closing plan emitted");
    ok($asm_idx < $plan_idx, "assembled subtest recorded before the plan that closed it");
};

subtest open_subtest_closed_at_process_exit => sub {
    my $as = Test2::Harness2::Collector::Assembler->new;

    # A streamed TAP subtest with no explicit close brace -- the child exits
    # while it is still open. The synthetic process-exit event must force the
    # buffered subtest closed so it is not lost from the log.
    my @out = run(
        $as,
        tap_open('st', 0),
        tap_assert(1, number => 1, name => 'child a', nested => 1),
    ...
    my $asm = assembled(@out);
    ok($asm, "the open subtest is force-closed and emitted at process exit");
    is($asm->facet_data->{parent}{details}, 'st', "the force-closed subtest is named");
    is(scalar(@{$asm->facet_data->{parent}{children}}), 1, "its buffered child is preserved");

    my ($exit) = grep { $_->facet_data->{harness_process_exit} } @out;
    ok($exit, "the process-exit event still passes through");
};
