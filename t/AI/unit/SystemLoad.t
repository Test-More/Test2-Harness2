...
subtest linux_guest_not_double_counted => sub {
    # guest / guest_nice (fields 8,9) are already folded into user/nice; summing
    # only fields 0..7 keeps busy% correct. Here total over fields 0..7 = 1150,
    # idle (idle+iowait) = 1000; bump total by 100 busy with a +500 guest tail that
    # must NOT count.
    my $src = linux_src('/proc/stat' => "cpu 100 0 50 1000 0 0 0 0 0 0\n");
    $src->sample;
    $src->{files}{'/proc/stat'} = "cpu 200 0 50 1000 0 0 0 0 500 0\n";
    my $b = $src->sample;
    is($b->{cpu_pct}, within(100, 0.01), "guest tail excluded -> 100% busy over the 100-tick delta");
};
