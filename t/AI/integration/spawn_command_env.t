...
my $script = "$dir/env.pl";
open my $sfh, '>', $script or die $!;
print $sfh 'print "FOO=$ENV{FOO}\nBAR=$ENV{BAR}\n"; exit 0;', "\n";
close $sfh;
...
yath(
    command => 'spawn',
    args    => ["--workdir=$dir", '-s', 'BASE', '-e', 'FOO', '-E', 'BAR=explicit', '--', $script],
    exit    => 0,
    test    => sub {
        like($_[0]->{output}, qr/FOO=from-parent/, '-e FOO copied');
        like($_[0]->{output}, qr/BAR=explicit/,    '-E BAR set');
    },
);
delete $ENV{FOO};
