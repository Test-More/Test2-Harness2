...
subtest 'flavor parse' => sub {
    is(
        [App::Yath2::Command::qdb::_parse_flavor('mariadb')],
        ['mariadb', U()],
        'bare flavor',
    );
    is(
        [App::Yath2::Command::qdb::_parse_flavor('postgres')],
        ['postgres', U()],
        'bare postgres',
    );
    is(
        [App::Yath2::Command::qdb::_parse_flavor('postgresql')],
        ['postgresql', U()],
        'postgresql passes through (caller normalises later)',
    );
    is(
        [App::Yath2::Command::qdb::_parse_flavor('postgres-15')],
        ['postgres', '15'],
        'postgres-15',
    );
    is(
        [App::Yath2::Command::qdb::_parse_flavor('mariadb-12.2')],
        ['mariadb', '12.2'],
        'mariadb-12.2',
    );
    is(
        [App::Yath2::Command::qdb::_parse_flavor('MySQL-9.7')],
        ['mysql', '9.7'],
        'flavor lower-cased',
    );
};
...
subtest 'flavor meta' => sub {
    my @postgres = App::Yath2::Command::qdb::_flavor_meta('postgres');
    ...
    is($postgres[4], 'postgres',   'postgres -> postgres flavor');
...
subtest 'version compare' => sub {
    is(App::Yath2::Command::qdb::_versioncmp('10.6', '12.2'),  -1, '10.6 < 12.2 numerically (not string)');
    is(App::Yath2::Command::qdb::_versioncmp('12.2', '10.6'),   1, '12.2 > 10.6 numerically');
    is(App::Yath2::Command::qdb::_versioncmp('17.9', '17.9'),   0, 'equal');
    is(App::Yath2::Command::qdb::_versioncmp('15',   '15.17'), -1, '15 < 15.17');
    is(App::Yath2::Command::qdb::_versioncmp('15.0', '15'),     0, '15.0 == 15 (missing components zero-padded)');
    is(App::Yath2::Command::qdb::_versioncmp('15.1', '15'),     1, '15.1 > 15');
};
...
subtest 'resolve_bin_dir' => sub {
    ...
    # No ~/dbs at all.
    is(
        App::Yath2::Command::qdb::_resolve_bin_dir('mariadb', undef),
        U(),
    ...
    is(
        App::Yath2::Command::qdb::_resolve_bin_dir('postgresql', undef),
        "$home/dbs/postgresql-17.9/bin",
        'highest postgresql selected',
    );
    is(
        App::Yath2::Command::qdb::_resolve_bin_dir('mariadb', '10.6'),
        "$home/dbs/mariadb-10.6/bin",
        'exact version match',
    );
    is(
        App::Yath2::Command::qdb::_resolve_bin_dir('postgresql', '15'),
        "$home/dbs/postgresql-15.17/bin",
        'prefix version match (15 -> 15.17)',
    );

    like(
        dies { App::Yath2::Command::qdb::_resolve_bin_dir('mariadb', '99') },
        qr/no \$HOME\/dbs entry matches 'mariadb-99'/,
        'explicit unmatched version croaks',
    );

    is(
        App::Yath2::Command::qdb::_resolve_bin_dir('mysql', undef),
        U(),
...
subtest 'parse args - missing flavor and missing archive' => sub {
    my $cmd = App::Yath2::Command::qdb->new(
        settings    => undef,
        args        => [],
        env_vars    => {},
        option_state => {},
