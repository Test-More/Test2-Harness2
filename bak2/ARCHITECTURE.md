...
- **Runner** — a row in the `runners` table created by
  ...
  signalling "no more runs are coming".
...
| `Test2::Formatter::Stream2` | In-test Test2 formatter that serialises events over `Atomic::Pipe` to its test-job collector. Lives outside `Test2::Harness2` because it loads inside test processes. Copied wholesale from `reference/old3`. |
...
- `$h->insert($table, \%row1, \%row2, …)` → list of newly-inserted
  row objects. Implementations must optimise multi-row inserts —
...
service stdout / stderr
  → Parser    (frames → events; raw lines → STDOUT/STDERR events; TAP not recognised)
...
- `Test2::Harness2::Collector::Parser::TAPParser` — subclass of
  `IOParser` that additionally recognises TAP and converts TAP
...
- `finalize` — called at the very end. Closes any open files,
  finalises the database row, etc. Recorders are responsible for
...
- `SIGTERM` / `SIGINT` / `SIGQUIT` — forwarded to the collected
  ...
  keeps draining the pipes and finalising; the collected process
...
`SIGUSR1` is an optimisation, not a contract. On platforms that
...
Two tables drive service behaviour:
...
collector finalises and exits too. The launcher that started the
service reaps the collector.

## 7. Launchers

`Test2::Harness2::Role::Launcher` consumes `Role::Service` and adds
launch-specific behaviour: poll the `launches` table for rows
...
- Reaps exited collectors. When a collector exits the launcher
  ...
  already finalised its `collectors` row through its recorder;
...
launcher**, never through a preload or any other specialised
launcher. The point is to record a deterministic skip / fail
regardless of what state the rest of the launch fleet is in: a
broken preload, a saturated specialised launcher, or a resource
...
This replaces the legacy "harness rewrites `$ENV{TMPDIR}`" behaviour
...
state. It exists to amortise per-test startup cost (loading Moose,
...
Earlier rewrite attempts (and `reference/old3`) modelled preloads as
...
Per `new_plan`, reload functionality is a later stage of Part 1.
The expected shape:
...
- Invariant: exactly one of `content` / `local_path` is non-null
  ...
  `content` when the collector finalises.
...
Once finalised the row carries the full event stream without any
